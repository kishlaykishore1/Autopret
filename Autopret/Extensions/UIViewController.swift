...
extension UIViewController {
    ...
    //MARK: BackButton
    public func setBackButton(tintColor: UIColor = .white, isImage: Bool = false, _ image: UIImage =  #imageLiteral(resourceName: "back_arrow_white") ) {
    ...
    //MARK: Like Button
    public func setRightButton(tintColor: UIColor = .white, isImage: Bool = false, image: UIImage =  #imageLiteral(resourceName: "back_arrow_white")){
        let btn1 = UIButton(type: .custom)
        if isImage {
            btn1.setImage(image.imageWithColor(color: tintColor), for: .normal)
            ...
            btn1.setTitle("Passer", for: .normal)
            btn1.frame = CGRect(x: 0, y: 0, width: 60, height: 30)
        }
